...
void Firebox::RendererAPI::RenderQuad()
{
	...
	{
		SDL_WarpMouseInWindow(SDL_GL_GetCurrentWindow(), firstMouseX, firstMouseY);
		SDL_ShowCursor();
	}
	...
	m_GLShader.SetMat4("u_View", view);
	m_GLShader.SetMat4("u_Projection", projection);
	for (uint i = 0; i < 11; i++)
	{
		glm::mat4 model = glm::mat4(1.0f);
		model = glm::translate(model, cubePositions[i]);
		model = glm::rotate(model, glm::radians(0.0f), glm::vec3(1.0f, 0.3f, 0.5f));
		m_GLShader.SetMat4("u_Model", model);
