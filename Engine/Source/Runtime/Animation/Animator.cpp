...
Firebox::Animator::Animator(Animation* animation) : m_CurrentTime(0.0f), m_DeltaTime(0.0f), m_CurrentAnimation(nullptr)
{
	...
	m_FinalBoneMatrices.reserve(256);

	for (uint i = 0; i < 256; i++)
		m_FinalBoneMatrices.emplace_back(Mat4x4(1.0f));
}
...
void Firebox::Animator::Update(float deltaTime)
{
	...
	if (m_CurrentAnimation)
	{
		...
		CalculateBoneTransform(&m_CurrentAnimation->GetRootNode(), Mat4x4(1.0f));
	}	
}
...
void Firebox::Animator::CalculateBoneTransform(const AssimpNodeData* node, Mat4x4 parentTransform)
{
	...
	Mat4x4 globalTransform = parentTransform * nodeTransform;
