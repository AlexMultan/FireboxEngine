#include "VulkanAPI.h"
#include "Rendering/Backends/OpenGL/OpenGLDebugger.h"
#include "Utils/DebugTools.h"
#include "Utils/Assert.h"

Firebox::Vulkan::VulkanAPI::VulkanAPI()
{
	FB_CORE_INFO("Vulkan version: {0}", (const char*)glGetString(GL_VERSION));
}
...
void Firebox::Vulkan::VulkanAPI::Init()
{
...
void Firebox::Vulkan::VulkanAPI::ClearColor(const Vector4& color)
{
	glClearColor(color.r, color.g, color.b, color.a);
}

void Firebox::Vulkan::VulkanAPI::Clear()
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
}

void Firebox::Vulkan::VulkanAPI::SetViewport(uint x, uint y, uint width, uint height)
{
	glViewport(x, y, width, height);
}

void Firebox::Vulkan::VulkanAPI::DrawIndexed(const Ref<VertexArray>& vao, uint indexCount)
{
	vao->Bind();
...
void Firebox::Vulkan::VulkanAPI::SetDepthFunc(APIEnum func)
{
	glDepthFunc(ToGLEnum(func));
}

void Firebox::Vulkan::VulkanAPI::ClearDepth(float value)
{
	glClearDepth(value);
}

void Firebox::Vulkan::VulkanAPI::Clear(APIEnum func)
{
	glClear(ToGLEnum(func));
}

void Firebox::Vulkan::VulkanAPI::BindTexture(uint slot, uint textureID)
{
	glActiveTexture(GL_TEXTURE0 + slot);
	glBindTexture(GL_TEXTURE_2D, textureID);
}

void Firebox::Vulkan::VulkanAPI::BindTextureArray(uint slot, uint textureID)
{
	glActiveTexture(GL_TEXTURE0 + slot);
	glBindTexture(GL_TEXTURE_2D_ARRAY, textureID);
}

void Firebox::Vulkan::VulkanAPI::BindFramebuffer(APIEnum target, uint framebuffer)
{
	glBindFramebuffer(ToGLEnum(target), framebuffer);
}

void Firebox::Vulkan::VulkanAPI::UnbindFramebuffer(APIEnum target)
{
	glBindFramebuffer(ToGLEnum(target), 0);
}

void Firebox::Vulkan::VulkanAPI::BufferSubData(APIEnum target, uint offset, size_t size, const void* data)
{
	glBufferSubData(ToGLEnum(target), offset, size, data);
}

void Firebox::Vulkan::VulkanAPI::CullFace(APIEnum target)
{
	glCullFace(ToGLEnum(target));
}

GLenum Firebox::Vulkan::VulkanAPI::ToGLEnum(APIEnum value)
{
	switch (value)
	{
		// Masks
	...
	FB_ASSERT(false, "Unsupported APIEnum value in VulkanAPI::ToGLEnum");
	return 0;
}
